...
final class ExportManager {
    ...
    func createExportBundle() throws -> [URL] {
        ...
        let root = AppPaths.protectedDataDirectory.appendingPathComponent(
            "Exports",
            isDirectory: true
        )
        ...
        try writeDailyMetricsCSV(
            to: directory.appendingPathComponent(
                "daily_metrics.csv"
            )
        )
        try writeSymptomsCSV(
            to: directory.appendingPathComponent(
                "symptoms.csv"
            )
        )
        try writeOrthostaticCSV(
            to: directory.appendingPathComponent(
                "orthostatic_sessions.csv"
            )
        )
        try writeMedicationsCSV(
            to: directory.appendingPathComponent(
                "medications.csv"
            )
        )
        ...
        AppLogger.shared.info(
            "Export bundle created: \(directory.lastPathComponent)"
        )
    ...
    private func writeDailyMetricsCSV(
        to url: URL
    ) throws {
        let headers = [
            "day_start",
    ...
    private func writeOrthostaticCSV(
        to url: URL
    ) throws {
        let iso =
            ISO8601DateFormatter()
    ...
    private func writeMedicationsCSV(
        to url: URL
    ) throws {
        let iso =
            ISO8601DateFormatter()
        ...
        let payload:
            [String: Any] = [
                ...
                "coverage_last_7_complete_days":
                    coverage,
                "note":
                    "Trend review only. Not a medical diagnosis."
            ]
    ...
    private func optionalNumberString(
        _ value: Double?
    ) -> String {
